...
exports.findAll = (req, res) => {
    ...
    Sucursal.findAll({ where: condition })
        ...
        .catch(err => {
            res.status(500).send({
                message:
                    err.message || "Some error occurred while retrieving sucursal."
            });
        });
};
...
exports.findOne = (req, res) => {
    ...
    Sucursal.findByPk(id)
        ...
        .catch(err => {
            res.status(500).send({
                message: "Error retrieving Sucursal with id=" + id
            });
        });
};
...
exports.update = (req, res) => {
    ...
    Sucursal.update(req.body, {
        ...
        .then(num => {
            if (num == 1) {
                res.send({
                    message: "Sucursal was updated successfully."
                });
            } else {
                res.send({
    ...
    Sucursal.destroy({
        ...
        .then(num => {
            if (num == 1) {
                ...
                res.send({
                    message: `Cannot delete Client with id=${id}. El sucursal no fue encontado!`
                });
            }
        })
        .catch(err => {
...
// find all active Client, basado en el atributo status vamos a buscar que solo los sucursals activos
exports.findAllStatus = (req, res) => {
    Sucursal.findAll({ where: { status: true } })
        .then(data => {
            res.send(data);
        })
        .catch(err => {
